...
if "!backup_taPartitionName!" == "-1" goto:eof
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c 'OP_ID='">tmpbak\backup_matchOP_ID
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c 'OP_NAME='">tmpbak\backup_matchOP_Name
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c 'ROOTING_ALLOWED='">tmpbak\backup_matchRootingStatus
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c -i 'S1_Boot'">tmpbak\backup_matchS1_Boot
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c -i 'S1_Loader'">tmpbak\backup_matchS1_Loader
	...
	tools\adb.exe shell %su% "%BB% cat /dev/block/%1 | %BB% grep -s -m 1 -c -i 'S1_HWConf'">tmpbak\backup_matchS1_HWConf
...
echo =======================================
tools\adb.exe shell %su% "%BB% ls -l %PARTITION_BY_NAME% | %BB% awk '{print \$11}'">tmpbak\backup_defaultTA
set /p backup_defaultTA=<tmpbak\backup_defaultTA
tools\adb.exe shell %su% "if [ -b '!backup_defaultTA!' ]; then echo '1'; else echo '0'; fi">tmpbak\backup_defaultTAvalid
...
if "!backup_defaultTAvalid!" == "1" (
	...
	tools\adb.exe shell %su% "%BB% cat /proc/partitions | %BB% awk '{if (\$3<=9999 && match (\$4, \"'\"mmcblk\"'\")) print \$4}'">tmpbak\backup_potentialPartitions
...
echo =======================================
tools\adb.exe shell %su% "%BB% md5sum !partition! | %BB% awk {'print \$1'}">tmpbak\backup_currentPartitionMD5
tools\adb.exe shell %su% "%BB% dd if=!partition! of=/sdcard/backupTA.img"

echo.
echo =======================================
echo  INTEGRITY CHECK
echo =======================================
tools\adb.exe shell %su% "%BB% md5sum /sdcard/backupTA.img | %BB% awk {'print \$1'}">tmpbak\backup_backupMD5
...
tools\adb.exe shell %su% "%BB% date +%%Y%%m%%d.%%H%%M%%S">tmpbak\TA.timestamp
...
tools\adb.exe shell rm /sdcard1/backupTA.img > nul 2>&1
goto:eof