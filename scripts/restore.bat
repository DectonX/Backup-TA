...
if exist tmpbak\TA.blk (
	...
	tools\adb.exe shell %su% "%BB% ls -l %PARTITION_BY_NAME% | %BB% awk '{print \$11}'">tmpbak\restore_defaultTA
	set /p restore_defaultTA=<tmpbak\restore_defaultTA
	tools\adb.exe shell %su% "if [ -b '!restore_defaultTA!' ]; then echo '1'; else echo '0'; fi">tmpbak\restore_defaultTAvalid
...
echo =======================================
tools\adb.exe shell %su% "%BB% md5sum !partition! | %BB% awk {'print \$1'}">tmpbak\restore_currentPartitionMD5
...
echo =======================================
tools\adb.exe shell %su% "%BB% dd if=!partition! of=/sdcard/revertTA.img && %BB% sync && %BB% sync && %BB% sync && %BB% sync"
if NOT "!errorlevel!" == "0" goto onRestoreFailed
tools\adb.exe shell %su% "%BB% ls -l /sdcard/revertTA.img | %BB% awk {'print \$5'}">tmpbak\restore_revertTASize
...
echo  INTEGRITY CHECK
echo =======================================
tools\adb.exe shell %su% "%BB% ls -l /sdcard/restoreTA.img | %BB% awk {'print \$5'}">tmpbak\restore_pushedBackupSize
tools\adb.exe shell %su% "%BB% md5sum /sdcard/restoreTA.img | %BB% awk {'print \$1'}">tmpbak\restore_pushedBackupMD5
if NOT "!errorlevel!" == "0" goto onRestoreFailed
...
if NOT exist tmpbak\TA.serial (
	tools\adb.exe shell %su% "%BB% cat /sdcard/restoreTA.img | %BB% grep -m 1 -o !restore_serialno!">tmpbak\restore_backupSerial
...
if NOT "%restore_dryRun%" == "1" (
	tools\adb.exe shell %su% "%BB% dd if=/sdcard/restoreTA.img of=!partition! && %BB% sync && %BB% sync && %BB% sync && %BB% sync"
	if NOT "!errorlevel!" == "0" goto onRestoreFailed
) else (
	echo --- dry run ---
)
tools\adb.exe shell %su% "rm /sdcard/restoreTA.img"

echo.
echo =======================================
echo  COMPARE NEW TA PARTITION WITH BACKUP
echo =======================================
tools\adb.exe shell %su% "%BB% md5sum !partition! | %BB% awk {'print \$1'}">tmpbak\restore_restoredMD5
if NOT "%restore_dryRun%" == "1" (
...
echo =======================================
if NOT "%restore_dryRun%" == "1" (
	tools\adb.exe shell %su% "%BB% dd if=/sdcard/revertTA.img of=%partition% && %BB% sync && %BB% sync && %BB% sync && %BB% sync"
)
...
echo =======================================
tools\adb.exe shell %su% "%BB% md5sum !partition! | %BB% awk {'print \$1'}">tmpbak\restore_revertedMD5
if NOT "%restore_dryRun%" == "1" (
